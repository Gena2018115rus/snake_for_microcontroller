...
#define B(b7, b6, b5, b4, b3, b2, b1, b0) ((b7 << 7) | (b6 << 6) | (b5 << 5) | (b4 << 4) | (b3 << 3) | (b2 << 2) | (b1 << 1) | (b0 << 0))
#define TRUE 1
...
ISR(TIMER1_COMPA_vect) // прерывание таймера 1
{
	// сюда мы будем попадать при срабатывании (в нашем случае единственного настроенного) таймера (работа таймера подробнее описана ниже)
	 ...
	 unsigned int OCR = ADC_convert() * 64 /*= OCR1AL | (OCR1AH << 8)*/;
...
void loadbit(unsigned char bit)
{
	// D0 - импульс // D1 - затвор // D2 - данные // D3 - speaker
	PORTD = (PORTD & B(1,1,1,1,1,0,1,1)/*0b11111011*/) | ((bit & 1) << 2);
	PORTD |= 1;
	PORTD &= ~1;
}
...
ISR(PCINT0_vect) // прерывание кнопок
{
	...
	if ((PINB & B(0,0,1,1,1,1,0,0)/*0b00111100*/) < (prevPINB & B(0,0,1,1,1,1,0,0)/*0b00111100*/)) // т.е. "если хоть одна из кнопок опустилать"
	{
		...
		if ((prevPINB & B(0,0,0,0,0,1,0,0)/*0b00000100*/) != (PINB & B(0,0,0,0,0,1,0,0)/*0b00000100*/)) // влево
		{
...
{
	PORTD |= 1 << 7;
	PORTC |= ~B(1,1,0,1,0,0,0,0)/*0b11010000*/; // и включаем на катодах
	PORTB |= ~B(1,1,1,1,1,1,0,0)/*0b11111100*/; // две строки, т.к. катоды разбросаны на два порта (т.е. надо работать с двумя регистрами)
...
int main(void)
{
	...
	PCMSK0 |= B(0,0,1,1,1,1,0,0)/*0b00111100*/; // второй, 3, четвёртой и пятой ногах
	...
	DDRC |= B(0,0,1,0,1,1,1,1)/*0b00101111*/; // катоды 0 - 5 на выход // когда на вход - к земле не притянуть
	DDRB |= B(0,0,0,0,0,0,1,1)/*0b00000011*/; // катоды 6 и 7 на выход
	PORTC &= B(1,1,0,1,0,0,0,0)/*0b11010000*/; // тут тоже пока выключу // катоды 0 - 5
	PORTB &= B(1,1,1,1,1,1,0,0)/*0b11111100*/; // и 6 - 7
	
	DDRB &= B(1,1,0,0,0,0,1,1)/*0b11000011*/; // кнопки на вход
	PORTB |= B(0,0,1,1,1,1,0,0)/*0b00111100*/; // подтягиваем к ним резисторы на 20-50 килоОм
